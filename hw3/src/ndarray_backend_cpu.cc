...
size_t calcindex(std::vector<uint32_t> indices, std::vector<uint32_t> strides, size_t offset) {
    size_t res = offset;
...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
  ...
  int ndim = shape.size();
  for (size_t i = 0; i < ndim; i ++) indices.push_back(0);
  bool finish = false;
  while(1) {
      out->ptr[cnt++] = a.ptr[calcindex(indices, strides, offset)];
      indices[ndim-1] ++;
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
  ...
  bool finish = false;
  while(1) {
      out->ptr[calcindex(indices, strides, offset)] = a.ptr[cnt++];
      indices[ndim-1] ++;
      for (size_t i = ndim - 1; i >= 0; i --) {
          if (indices[i] == shape[i]) {
              if (i == 0) {
...
void ScalarSetitem(const size_t size, scalar_t val, AlignedArray* out, std::vector<uint32_t> shape,
  ...
  bool finish = false;
  while(1) {
      out->ptr[calcindex(indices, strides, offset)] = val;
      indices[ndim-1] ++;
      for (size_t i = ndim - 1; i >= 0; i --) {
          if (indices[i] == shape[i]) {
              if (i == 0) {
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for (size_t i = 0; i < out->size; i ++) {
      ...
      for (size_t j = 0; j < reduce_size; j ++) {
          if (first) first = false, max = a.ptr[i*reduce_size+j];
