...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    ### BEGIN YOUR SOLUTION
    input_dim, hidden_dim, num_classes = X.shape[1], W1.shape[1], W2.shape[1]
    for i in range(int(X.shape[0] / batch)):
        ...
        y_onehot_batch = ndl.Tensor(np.eye(num_classes)[y_batch])
        # forward
        z = ndl.relu(X_batch @ W1) @ W2
        loss = softmax_loss(z, y_onehot_batch)
        # backward
        loss.backward()
