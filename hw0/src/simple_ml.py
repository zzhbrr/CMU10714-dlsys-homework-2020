...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    return np.mean(np.log(np.sum(np.exp(Z), axis=1)) - Z[np.arange(0, Z.shape[0]), y])
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(int(X.shape[0] / batch)):
        ...
        exp_xtheta = np.exp(X_batch @ theta)
        Z = exp_xtheta / np.sum(exp_xtheta, axis=1, keepdims=True)
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(int(X.shape[0] / batch)):
        ...
        Z2_exp = np.exp(Z1 @ W2)
        # backward
        G2 = Z2_exp / np.sum(Z2_exp, axis=1, keepdims=True) - np.eye(num_classes)[y_batch]
        G1 = (Z1>0) * (G2@W2.T)
