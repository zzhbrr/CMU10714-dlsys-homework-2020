...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self._shape) != prod(new_shape):
            # print('ndarray.reshape: self._shape', self._shape)
            # print('ndarray.reshape: new_shape', new_shape)
            raise ValueError
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = tuple([self._shape[i] for i in list(new_axes)])
        new_strides = tuple([self._strides[i] for i in list(new_axes)])
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        ### BEGIN YOUR SOLUTION
        if self.ndim == 1: # 一维向量默认为行向量
            assert self._shape[0] == 1 or self._shape[0] == new_shape[len(new_shape)-1]
            if len(new_shape) == 1:
                new_strides = (0, )
            else:
                new_strides = tuple([0 if i!=len(new_shape)-1 else 1 for i in range(len(new_shape))])
            return self.as_strided(new_shape, new_strides)
        else:
            for i in range(self.ndim):
                if self._shape[i] != 1 and new_shape[i] != self._shape[i]:
                    raise AssertionError
            new_strides = tuple([self._strides[i] if self._shape[i] != 1 else 0 for i in range(self.ndim)])
            return self.as_strided(new_shape, new_strides)
        ### END YOUR SOLUTION
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = tuple([int((sl.stop-sl.start+sl.step-1)/sl.step) for sl in idxs])
        new_offset = int(np.sum(np.array([idxs[i].start * self._strides[i] for i in range(self.ndim)])))
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        # new_strides: axes对应的x-1即可
        # new_offset: axes对应的位置加上(shape[axis]-1)*strides[axis]
        new_strides = list(NDArray.compact_strides(self._shape))
        offsets = [(self._shape[i]-1)*new_strides[i] for i in range(self.ndim)]
        new_offset = 0
        for axis in axes:
            new_offset += (self._shape[axis]-1)*new_strides[axis]
            new_strides[axis] *= -1
        res = NDArray.make(self._shape,strides=tuple(new_strides), device=self.device, handle=self.compact()._handle, offset=new_offset)
        return res.compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for tp in axes:
            assert len(tp) == 2
        new_shape = [self._shape[i]+axes[i][0]+axes[i][1] for i in range(self.ndim)]
        res = NDArray.make(new_shape, device=self.device)
        res.fill(0)
        sl = [slice(axes[i][0], new_shape[i]-axes[i][0], 1) for i in range(self.ndim)]
        res[tuple(sl)] = self
        return res
        ### END YOUR SOLUTION
