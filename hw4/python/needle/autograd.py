...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        # print('node.grad: ', node.grad.shape, ' node id: ', id(node))
        # print(node.op)
        if not node.is_leaf():
            # partial_adjoint_to_node = node.op.gradient(node.grad, node)
            partial_adjoint_to_node = node.op.gradient_as_tuple(node.grad, node) # 草草草坑死我了，这个地方卡了我半天
            # print('        inputs grad shape', partial_adjoint_to_node[0].shape, ' inputs id: ', id(node.inputs[0]))
            # if len(node.inputs)>=2:
            #     print('        inputs grad shape', partial_adjoint_to_node[1].shape, ' inputs id: ', id(node.inputs[0]))
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    if node in visited:
        return 
    visited.append(node)
    for nxt in node.inputs:
        topo_sort_dfs(nxt, visited, topo_order);
