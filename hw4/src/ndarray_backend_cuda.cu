...
__device__ size_t calcindex(const CudaVec& indices, const CudaVec& strides, size_t offset) {
    size_t res = offset;
    for (size_t i = 0; i < indices.size; i ++) 
        res += indices.data[i] * strides.data[i];
    return res;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    ...
    for(size_t i = shape.size; i >= 1; i --) {
        indices.data[i-1] = gid_tmp % shape.data[i-1];
        gid_tmp = gid_tmp / shape.data[i-1];
    }
    size_t index = calcindex(indices, strides, offset);
    out[gid] = a[index];
  }
  /// END YOUR SOLUTION
}
...
// 使用Shared Memory, Block中的线程维度要相同
...
__global__ void MatmulKernel(const scalar_t* A, const scalar_t* B, scalar_t* C, int row_a, int col_a, int row_b, int col_b) {
    ...
    float sum = 0;
    int num_iter = ceil((float)col_a / thread_num_x); // 迭代的次数
    for (int i = 0; i < num_iter; i ++) {
        if (i*blockDim.x+threadIdx.x < col_a && row < row_a) // 如果已经超出矩阵A的范围，则不更新
            As[threadIdx.y][threadIdx.x] = A[row*col_a + i*blockDim.x + threadIdx.x];
        else As[threadIdx.y][threadIdx.x] = 0;
        
        if (i*blockDim.y+threadIdx.y < row_b && col < col_b) // 如果已经超出矩阵B的范围，则不更新
        ...
        #pragma unroll
        for (int k = 0; k < blockDim.x; k ++) sum += As[threadIdx.y][k] * Bs[k][threadIdx.x]; // 对目前这一块的矩阵进行计算
        __syncthreads();
    }
    if (row < row_a and col < col_b)
        C[row*col_b+col] = sum;
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  if (gid < size) {
      ...
      for (size_t i = 0; i < reduce_size; i ++) {
        if (first) max = a[gid * reduce_size + i], first = false;
