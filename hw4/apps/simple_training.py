...
def train_cifar10(model, dataloader, n_epochs=1, optimizer=ndl.optim.Adam,
    ...
    for i in range(n_epochs):
        acc, loss = epoch_general_cifar10(dataloader, model, 
...
def epoch_general_ptb(data, model, seq_len=40, loss_fn=nn.SoftmaxLoss(), opt=None,
    ...
    for i in range(0, nbatch, seq_len):
        ...
        y_hat, h = model(x, h)
            # y_hat shape of (seq_len * bs, output_size)
        if isinstance(h, tuple):
            h = (h[0].detach(), h[1].detach())
        else:
            h = h.detach()
        loss = loss_fn(y_hat, y)
        correct += (y_hat.numpy().argmax(axis=1) == y.numpy()).sum()
        if opt:
            loss.backward()
            opt.step()
        loss_sum += loss.numpy() * y.shape[0]
        print('acc:{:.3f}, loss:{:.3f}'.format(correct/tot, loss_sum/tot))
...
def train_ptb(model, data, seq_len=40, n_epochs=1, optimizer=ndl.optim.SGD,
    ...
    for epoch in range(n_epochs):
        acc, loss = epoch_general_ptb(data, model, seq_len=seq_len, loss_fn=loss_fn(), opt=opt,
                        clip=clip, device=device, dtype=dtype)
        print('epoch: {}, acc: {:.3f}, loss: {:.3f}'.format(epoch, acc, loss))
    return acc, loss
    ### END YOUR SOLUTION
